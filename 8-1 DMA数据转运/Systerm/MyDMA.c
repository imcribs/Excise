#include "stm32f10x.h"                  // Device header

uint16_t MyDMA_Size;
void MyDMA_Init(uint32_t AddrA,uint32_t AddrB,uint32_t Size)
{
	
	MyDMA_Size = Size;
	//AHB时钟使能
	 RCC_AHBPeriphClockCmd(RCC_AHBPeriph_DMA1,ENABLE);  
	
	//初始化DMA
	DMA_InitTypeDef DMA_InitStructure;
	DMA_InitStructure.DMA_PeripheralBaseAddr = AddrA;     //外设基地址
	DMA_InitStructure.DMA_PeripheralDataSize= DMA_PeripheralDataSize_Byte;  // 外设数据位宽
	DMA_InitStructure.DMA_PeripheralInc= DMA_PeripheralInc_Enable ;    // 是否递增
	DMA_InitStructure.DMA_MemoryBaseAddr= AddrB;    //内存的基地址
	DMA_InitStructure.DMA_MemoryDataSize= DMA_MemoryDataSize_Byte;   // 内存的数据位宽
	DMA_InitStructure.DMA_MemoryInc= DMA_MemoryInc_Enable;    //是否递增
	DMA_InitStructure.DMA_BufferSize= Size;      //DMA传输数据单元总数 //传输寄存器的大小  //理论最大是65535 ，但是内存达不到
	DMA_InitStructure.DMA_DIR=DMA_DIR_PeripheralSRC  ;        //传输的方向   //
	DMA_InitStructure.DMA_M2M= DMA_M2M_Enable;     //软件触发
	DMA_InitStructure.DMA_Mode= DMA_Mode_Normal ;      //是否开启重装寄存器
	DMA_InitStructure.DMA_Priority=  DMA_Priority_Medium ;     //优先级
	DMA_Init(DMA1_Channel1,&DMA_InitStructure);
	
	
	DMA_Cmd(DMA1_Channel1,DISABLE);
}


void MyDMA_Transfer(void)
{
	DMA_Cmd(DMA1_Channel1,DISABLE);
	DMA_SetCurrDataCounter(DMA1_Channel1,MyDMA_Size);
	DMA_Cmd(DMA1_Channel1,ENABLE);
	
	while (DMA_GetFlagStatus(DMA1_FLAG_TC1) == RESET);//循环判断是否为真，如果置1，则是转换完成，为假，跳出循环
	DMA_ClearFlag(DMA1_FLAG_TC1);
}	
